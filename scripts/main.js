...
const EPISODE_1 = {
  ...
  exec: [
    ...
    () => {
      ...
      GAME.addAndUpdate(obj);
      GAME.HIDDEN ? null : (GAME.STATE = "MAVJUDOT");
    },
    // LOOK
    () => {
      const obj = {
    ...
    () => {
      ...
      GAME.STATE === "MASHINA"
        ? null
        : ((GAME.HIDDEN = true), (GAME.STATE = "MASHINA"));
    },
    // lMavjudot
    () => {
      const obj = {
    ...
    () => {
      const obj = {
        ...EPISODE_1,
        msg: `
        <img style="width: 100%" src="./res/img.jpg" alt="Paranormal Tadqiqotchilar..."><br>
        Paranormal Tadqiqotchilar, Ed Warren va uning rafiqasi Lorraine (Harrisville).
        Ed tunda yo'lga otlanadi va ushbu voqea ro'y beradi.
        `,
      };
      GAME.addAndUpdate(obj);
    },
...
const GAME = {
  ...
  snippets: {
    ...
    chooseEpz: `Bo'limlardan birini tanlang <span style='font-style: italic'>(raqam bilan)</span>:
            <p class="opt"><span class="auto">[1]</span> Annabel (test)</p>
            ...
            <p class="opt"><span class="auto">[8]</span> To'liq Ekranga O'tish</p>
            `,
    aboutSection: `<p>Dasturlovchi: Humoyun</p>
...
setInterval(blinkAnimation, 1000 * 60);
...
function addArray(obj, _text, _type) {
  ...
  if (
    ...
    const index = lastBotResponse.responses.indexOf(_text.trim().toUpperCase());
...
$M({
  "#user-input": {
    onkeyup: (_e) => {
      if (_e.keyCode === 13 && _e.target.value && _e.target.value.length < 40) {
  ...
  switch (_e.key) {
    case "ArrowUp":
      ++index > GAME.keyboardHistory.length - 1
        ? (index = GAME.keyboardHistory.length - 1)
        : index;
      document.querySelector("#user-input").value = GAME.keyboardHistory[index];
      break;
    case "ArrowDown":
